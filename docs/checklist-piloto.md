...
| ⬜ P14 | **Plantillas de WhatsApp aprobadas por Meta**, seis: recordatorio 24 h, recordatorio del día, confirmación de cita del portal y cancelación —cuerpo con cuatro variables en orden: código, servicio, fecha, hora—; más reapertura de conversación y contacto inicial, con UNA sola variable, el nombre | Recordatorios, confirmación del portal (RN-05, RN-10.3), bandeja (RN-10.4) | Sin ellas, el envío fuera de la ventana de 24 h no sale y queda anotado en auditoría. Los nombres se cargan en Administración → Reglas, sin desplegar. El texto propuesto para la de contacto inicial está en `docs/rn-10-4-contacto-con-el-portal.md`; quien no quiera tramitarla puede pegar ahí el nombre de la de reapertura. Ver `docs/changelog-fase8.md` |
...
- 🟡 **Evaluación de la IA.** El arnés existe y corre: `npm run evaluar -w @provivir/api`
  ...
  **Línea base medida** contra `gpt-5-mini` con `--repeticiones 3` (2026-08-23): **45/47 en la
  última corrida completa, sin fallos críticos** (`seguridad` 5/5, `privacidad` 3/3), latencia
  mediana 5,0 s. Los dos fallos y el caso nuevo se remidieron aparte y pasan 3/3, así que el
  conjunto está en **48 casos sin fallos conocidos**. En dos corridas completas, los cinco fallos
  fueron expectativas mal planteadas y ninguno conducta del bot.
  ⬜ **Falta una corrida de las 48 de un tirón** para registrarla como línea base cerrada
  (≈144 llamadas). La comparación de modelos del ADR A5 es de 31 casos y del prompt anterior:
  no sirve de referencia.
